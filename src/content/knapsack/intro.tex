...
Our algorithm retains the same running time of $O(n^2)$ as the greedy algorithm and the fast algorithm of \cite{khuller1999budgeted}, but achieves an improved approximation guarantee of $\approx 0.4534$ (whereas the fast algorithm of \cite{khuller1999budgeted} provided a guarantee of only $1-e^{-\nicefrac{1}{2}}\approx 0.393$).
...
Let $k\geq \max \{2, \frac{1}{3\log_2(\nicefrac[]{1}{r} -1 ) }+1 \}$.
Then there exists an algorithm that runs in time $3n^2/2+n$ plus $k$ times the running time of $\mathcal{A}$ and achieves an approximation of $(1-e^{-\alpha^*})$, where:
\begin{enumerate}
\item $\alpha ^* = \max _{\nicefrac[]{2}{3}\leq \alpha \leq \ln{2}} \left\{ \min \left\{ 1-e^{-\alpha},D(\alpha)+(1-r)\left( \frac{2+\varepsilon ^*}{1+\varepsilon ^*} (1-D(\alpha)) -1\right)\right\}\right\}$.
\item $ D(\alpha)=(1-e^{-\alpha})/B(\alpha)$.
\item $\varepsilon ^* = 2^{-\frac{\log _2 {A(\alpha)}}{k-1}}-1$.
\item $ A(\alpha) = \frac{1}{1-e^{-\alpha}}-\frac{1}{B(\alpha)}$.
\item $ B(\alpha)=1-e^{-\frac{\alpha}{2\alpha - 1}}$.
\end{enumerate}
\end{theorem}
